...
#include <string.h>
#include <stdio.h>
#include <stdlib.h> // 需要包含 stdlib 用于 exit()
...
// [优化]: 消除 malloc 动态内存分配，直接写入传入的 dest 缓冲区
...
// [优化]: 彻底移除 sscanf，使用高效的字符位运算，消除巨大的格式化开销
void swap_endian_words(const char *hex_words, uint8_t *output)
{
    ...
    for (size_t i = 0; i < binary_length; i += 4)
    {
        for (int j = 0; j < 4; j++)
        {
            uint8_t high_nibble = hex2val(hex_words[(i + j) * 2]);
            uint8_t low_nibble  = hex2val(hex_words[(i + j) * 2 + 1]);
            output[i + (3 - j)] = (high_nibble << 4) | low_nibble;
        }
    }
}
